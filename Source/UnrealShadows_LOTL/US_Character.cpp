...
AUS_Character::AUS_Character()
{
    ...
    /*NOTES: CreateDefaultSubobject()<T> is a function that is used to create a new subobject of a class that is owned by another object
     SUBOBJECT: a component or a member variable of an object , and the method is typically called within an object's constructor to initalize its subobjects
     SetUpAttachment () method will reparent a component to another one*/
    CameraBoom =
    ...
    /* NOTES: Here we are just changing some default values of the character Actor and its components
     * We are getting the character model and assigning it to SkeletalMeshComponent by means of FObjectFinder() utility method available in the ConstructorHelpers class
     */
...
{
	//Debug Message for now nothing to pick up
	GEngine->AddOnScreenDebugMessage(3, 5.f, FColor::Red,
		TEXT("Interact"));
}
...
void AUS_Character::BeginPlay()
{
	...
	/*NOTES: First line checks that the Controller is a PlayerController by means of Cast.
	 * Then the code will try to get an Enhanced Input Subsystem from the player and if successful add a mapping context to it
	 * From this point on all the actions declared inside the content will be "tracked" by the input system
	 */
...
void AUS_Character::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	/* NOTES: We are calling the BindAction() method on the input component pointer to bind each action to the corresponding method*/
	if (UEnhancedInputComponent* EnhancedInputComponent =
		Cast<UEnhancedInputComponent>(PlayerInputComponent))
	{
		EnhancedInputComponent->BindAction(MoveAction, ETriggerEvent::Triggered,
			this, &AUS_Character::Move);
		EnhancedInputComponent->BindAction(LookAction, ETriggerEvent::Triggered,
			this, &AUS_Character::Look);
		EnhancedInputComponent->BindAction(InteractAction, ETriggerEvent::Started,
			this, &AUS_Character::Interact);
		EnhancedInputComponent->BindAction(SprintAction, ETriggerEvent::Started,
			this, &AUS_Character::SprintStart);
		EnhancedInputComponent->BindAction(SprintAction, ETriggerEvent::Completed,
			this, &AUS_Character::SprintEnd);
